...
class RPPGVideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if face_detected_now and self.current_hr is not None and not np.isnan(self.current_hr):
            cv2.putText(img, f"HR: {self.current_hr:.1f} BPM", (30, 50), cv2.FONT_HERSHEY_DUPLEX, 1.2, (75, 75, 245), 2)
        else:
            cv2.putText(img, "Scanning Face...", (30, 50), cv2.FONT_HERSHEY_DUPLEX, 1.2, (0, 0, 255), 2)

        self.bufferIndex = (self.bufferIndex + 1) % self.bufferSize
        return av.VideoFrame.from_ndarray(img, format="bgr24")

st.title("rPPG Live Prototype")
webrtc_streamer(
    key="rppg-stream",
